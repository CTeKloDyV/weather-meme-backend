...
function getMemeByTemp(temp, baseUrl = process.env.BASE_URL || `http://localhost:${process.env.PORT || 5000}`) {
    let memesData;
    try {
    ...
    else if (temp >= 10) category = 'normal';
    else if (temp >= 0) category = 'cool';
    else category = 'cold';
